...
export default function ReservationSystem() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Validación de la cantidad de comensales
    if (guests <= 0) {
    ...
    const reservationData = {
      day: day.toISOString(),
      startTime: new Date(`1970-01-01T${startTime}:00Z`).toISOString(), // Convertir startTime a Date ISO
      tableNumber: currentReservation.tableNumber,
      guests, // Mantener el número como un número
    };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  id="day"
                  name="day"
                  value={currentReservation.day ? currentReservation.day.toISOString().split('T')[0] : ''}
                  onChange={(e) => handleChange('day', e.target.value)}
                  required
                />
              </div>
              ...
              {/* Botón de PayPal solo si tenemos el reservationId */}
              {reservIdentification && (
                <PayPalButton
                  reservIdentification={reservIdentification}  // Pasamos el 'reservIdentification'
                  onSuccess={(orderId) => {
                    toast.success(`Pago exitoso con ID: ${orderId}`);
                  }}
                />
              )}
